...
int have_guest_base;
static float bb_dists[MAP_SIZE];
static int bb_targets[MAX_TARGETS];
static int uaf_targets[MAX_UAF_TARGETS];
...
void cpu_loop(CPUX86State *env)
{
    ...
    for(;;) {
        cpu_exec_start(cs);
        trapnr = cpu_exec(cs, bb_dists, bb_targets, uaf_targets);
        cpu_exec_end(cs);
        ...
        case EXCP_DEBUG:
            ...
            break;
        case EXCP_ATOMIC:
            cpu_exec_step_atomic(cs, bb_dists, bb_targets, uaf_targets);
            break;
        default:
...
static void parse_distance(void) {
    char *line = NULL;
    size_t len = 0, read;
    FILE *fp = fopen(getenv("BB_DISTANCE_ENV_VAR"), "r");
    if (!fp) {
        (void) fprintf(stderr, "Cannot open basic block distance file!\n");
        exit(EXIT_FAILURE);
    }
    while ((read = getline(&line, &len, fp)) != -1) {
        char *str_addr = strtok(line, ",");
        u32 addr = strtoul(str_addr, NULL, 16);
        char *str_dist = strtok(NULL, "");
        float distance = (float) atof(str_dist);
        u32 cur_loc  = ((addr >> 4) ^ (addr << 8)) & (MAP_SIZE - 1);
        if (distance > 0) {
            printf("[parse_distance] addr: 0x%x, distance: %f\n", addr, distance);
            bb_dists[cur_loc] = distance;
        }
    }
    fclose(fp);
}
...
static int parse_targets(void) {
    ...
    if (!fp) {
        (void) fprintf(stderr, "Cannot open targets file!\n");
        exit(EXIT_FAILURE);
    }
    int i = 0;
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        // printf("[parse_targets] addr: 0x%x, fname: %s\n", addr, fname);
        bb_targets[i] = addr;
        i++;
    }
    fclose(fp);
...
static void parse_uaf_targets(void) {
    char *line = NULL;
    size_t len = 0, read;
    FILE *fp = fopen(getenv("UAF_ENV_VAR"), "r");
    if (!fp) {
        (void) fprintf(stderr, "Cannot open UAF targets file!\n");
        exit(EXIT_FAILURE);
    }
    int i = 0;
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        u32 addr = strtoul(str_addr, NULL, 16);
        char *fname = strtok(NULL, "");
        printf("[parse_uaf_targets] addr: 0x%x, fname: %s\n", addr, fname);
        uaf_targets[i] = addr;
        i++;
    }
    fclose(fp);

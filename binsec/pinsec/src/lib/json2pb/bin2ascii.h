...
inline std::string hex2bin(const std::string &s)
{
	...
	for (size_t i = 0; i < s.size(); i += 2) {
        char hi = lookup[(unsigned)s[i]];
        char lo = lookup[(unsigned)s[i+1]];
		if (0x80 & (hi | lo))
...
inline std::string bin2hex(const std::string &s)
{
	...
	for (size_t i = 0; i < s.size(); i++) {
		char hi = s[i] >> 4;
		char lo = s[i] & 0xf;
        r.push_back(lookup[(unsigned)hi]);
        r.push_back(lookup[(unsigned)lo]);
	}
	return r;
}
...
inline std::string b64_encode(const std::string &s)
{
    ...
    for (unsigned int i = 0; i < (3 - s.size() % 3) % 3; i++)
		r.push_back('=');
	return r;
}

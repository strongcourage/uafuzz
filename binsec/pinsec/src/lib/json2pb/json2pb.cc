...
public:
	...
	virtual ~j2pb_error() throw() { std::cout << _error << std::endl; };

	virtual const char *what() const throw () { std::cout << _error << std::endl;  return _error.c_str(); };
};
...
#undef _CONVERT
	...
	if (!jf) {
		std::cout << "Fail to convert to json" << std::endl;
...
#define _CONVERT(type, ctype, fmt, sfunc, afunc) 		\
		case FieldDescriptor::type: {			\
			...
			if (r) {  std::cout << "Failed to unpack: " << error.text << std::endl; throw j2pb_error(field, std::string("Failed to unpack: ") + error.text);} \
		...
		case FieldDescriptor::CPPTYPE_ENUM: {
			...
			if (json_is_integer(jf)) {
				...
				std::cout << "Not an integer or string" << std::endl;
				throw j2pb_error(field, "Not an integer or string");
			}
			if (!ev) {
				std::cout << "Enum value not found" << std::endl;
...
static void _json2pb(Message& msg, json_t *root)
{
	...
	if (!d || !ref) {
		std::cout << "No descriptor or reflection" << std::endl;
...
void json2pb(Message &msg, const char *buf, size_t size)
{
	...
	if (!json_is_object(root)) {
		std::cout << "Malformed JSON: not an object" << std::endl;
